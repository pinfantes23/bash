...
function helpPanel(){
 ...
 echo -e "\t${purpleColour}u)${endColour}${grayColour} Actualizar archivos de máquinas${endColour}"
}
...
function searchMachine(){
  ...
  if [ "$machineName_checker" ]; then
    ...
    echo -e "\n${yellowColour}[+]${endColour}${blueColour}Listando las propiedades de la máquina${endColour} $machineName"
...
function getYoutubeLink(){
  ...
  if [ $youtubeLink ]; then
    echo -e "\n$yellowColour[+] ${endColour}${grayColour}El tutorial de la máquina esta en el siguiente enlace:${endColour}${blueColour} $youtubeLink ${endColour}\n"
  else
    echo -e "\n${redColour}La máquina proporcionada no existe ${endColour}\n"
  fi
...
function getOSDifficultyMachines(){
  ...
  check_results="$(cat bundle.js | grep "dificultad: \"$difficulty\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column)"
  ...
  if [ "$check_results" ]; then
    ...
    echo -e "\n${redColour}[!] Se ha indicado un sistema operativo incorrecto. ${endColour}\n"
  fi
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    echo -e "\n${yellowColour}[+] ${endColour}${grayColour} A continuación se representan las máquinas donde se usa la técnica ${endColour} ${blueColour}${skill}${endColour} ${yellowColour}:${endColour}\n"
